...
`DownloadFileAsync()`, checks and sees if the files need to be 
...
Getting the base class requires several statements. 
The first statement, lines 4-7, retrieves all `ClassDeclarationSyntax` nodes
from the abstract syntax tree. The 
...
an interface that provides access to semantic information about a type. The 
...
the `ClassDeclarationSyntax` of the base class.
